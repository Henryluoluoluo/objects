function updateUser() {
	let aid = document.getElementById("updateaid").value;
	...
	let lend_num = document.getElementById("updatelend_num").value;
	let max_num = document.getElementById("updatemax_num").value;
	let reg = /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/;
	let myreg = /^[0-9]{1,20}$/;
	if (name == "" || aid == "" || username == "" || email == "" || phone == "" || password == "" || lend_num == "" || max_num == "") {
		alert("输入不能为空,请重新输入")
	} else if (lend_num < 0 || lend_num > 30) {
		...
		$.ajax({//把编号带到后台
			type: 'post',
			url: "http://localhost:8080/objects/updateUserServlet",
			async: false,
			data: { "aid": aid, "username": username, "name": name, "email": email, "phone": phone, "password": password, "lend_num": lend_num, "max_num": max_num },
