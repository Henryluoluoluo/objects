...
<%@ page import = "com.rain.bean.AdminBean,com.rain.dao.AdminDao,com.rain.bean.ObjectBean,com.rain.dao.ObjectDao" %>
	...
	<div class="container">
	    <div class="row">
	        ...
	        <div class="col-md-10">   
	            	...
	            	<div class="col-lg-12">
	                	<table id="data_list" class="table table-hover table-bordered" cellspacing="0" width="100%">
							...
							<tbody>
		                        ...
		                        <td><%= bean.getName() %></td>
		                        <td><%= bean.getNum() %></td>
								<td><button type="button" class="btn btn-info btn-xs" data-toggle="modal" onclick="borrowobject(<%= bean.getOid() %>,<%= bean.getNum() %>,<%=admindao.getNotReturn(admin.getUsername()) %>,<%=admin.getMax_num() %>)" >借出</button></td>
	                        </tbody>
	                        <%} %> 
	                    </table>
	                </div>
	    		...
	    		function borrowobject(oid,bnum,nRNum,MaxNum) {
	    			con=confirm("是否借出?"); 
	    			if(con==true){
		    			if(bnum == 0){
		    				con1 = alert("余量为0，无法借出");
		    			}
		    			else if(nRNum >= MaxNum){
		    				con2 = alert("已经超过可借出数，无法借出");
		    			}
		    			else{
		    				location.href = "/objects/borrowServlet?tip=1&oid="+oid;
		    			}
	    			}
	   		 	}
	    		</script>
